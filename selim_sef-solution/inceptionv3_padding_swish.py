...
def conv2d_bn(x,
    ...
    x = Activation('swish', name=name)(x)
    return x


def InceptionV3SameSwish(include_top=True,
                weights='imagenet',
                input_tensor=None,
                input_shape=None,
                pooling=None,
    ...
    if weights == 'imagenet':
        ...
        if input_shape[-1] > 3:
            ...
            conv1.name = 'conv1_more_channels'
        ...
        if input_shape[-1] > 3:
            ...
            inception_ori = InceptionV3(include_top=False, input_shape=(299, 299, 3))
            conv1_weights[:, :, :3, :] = inception_ori.layers[1].get_weights()[0][:, :, :, :]
            # random init
            conv1_weights[:, :, 3:, :] = model.get_layer('conv1_more_channels').get_weights()[0][:, :, 3:, :]
            model.get_layer('conv1_more_channels').set_weights((conv1_weights,))
            model.get_layer('conv1_more_channels').name = 'conv2d_1'
